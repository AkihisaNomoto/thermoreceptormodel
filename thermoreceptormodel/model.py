...
class ThermoreceptorModel:
    """
    A model representing thermal response of skin receptors.
    ...
    def __init__(self):
        # Version
        self.version = __version__  # Add version information
    ...
    def _prepare_dataframe(
        ...
        df["dR_warm"] = df["R_warm"].diff()  # Derivative of thermal response
        time_to_integrate = 20
        df["PSI"] = (
            df["dR_warm"].rolling(int(time_to_integrate / self.dt)).sum()
        )  # Integral of dRt over a window

        # Insert version information as the first column
